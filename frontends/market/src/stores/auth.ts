  ...
  actions: {
    ...
    async updateProfile(payload: ProfileUpdatePayload) {
      ...
      try {
        ...
        // Если бэкенд вернул обновлённого пользователя — используем его, иначе мержим локально и обновляем с сервера.
        if (updated && typeof updated === "object" && ("email" in updated || "id" in updated)) {
          this.user = { ...(this.user || ({} as User)), ...(updated as any) };
        } else {
          this.user = { ...(this.user || ({} as User)), ...(payload as any) };
          // Пробуем подтянуть свежие данные
          this.user = await apiMe().catch(() => this.user);
        }
      } catch (e: any) {
